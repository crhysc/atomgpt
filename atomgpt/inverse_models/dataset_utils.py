...
def train_on_responses_only(
    ...
    if hasattr(tokenizer, "image_processor") or hasattr(
        tokenizer, "tokenizer"
    ):
    ...
    if hasattr(trainer, "train_dataset") and trainer.train_dataset is not None:
        ...
        if isinstance(trainer.train_dataset, IterableDataset):
            trainer.train_dataset = trainer.train_dataset.map(
                ...
                batch_size=trainer.train_dataset._ex_iterable.batch_size,
                batched=True,
            )
        else:
            trainer.train_dataset = trainer.train_dataset.map(
                _train_on_responses_only, batched=True, num_proc=num_proc
            )
    pass
            ...
            for key, value in trainer.eval_dataset.items():
                ...
                if isinstance(trainer.eval_dataset, IterableDataset):
                    trainer.eval_dataset[key] = value.map(
                        _train_on_responses_only,
                        batch_size=trainer.eval_dataset._ex_iterable.batch_size,
                        batched=True,
                    )
                else:
                    trainer.eval_dataset[key] = value.map(
...
def standardize_data_formats(
    ...
    all_aliases = set(
        aliases_for_system + aliases_for_user + aliases_for_assistant
    )

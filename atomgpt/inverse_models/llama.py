...
from atomgpt.inverse_models._utils2 import Version, _get_dtype
from atomgpt.inverse_models.peft_utils import SKIP_QUANTIZATION_MODULES
...
from atomgpt.inverse_models.vision import FastBaseModel
...
def fix_prepare_inputs_for_generation(module):
    ...
    if hasattr(module, "prepare_inputs_for_generation"):
        module.prepare_inputs_for_generation = (
...
def fast_layernorm_compiled(layernorm, X):
    ...
    X = (
        Xbar
        * torch.rsqrt(
            Xbar.square().mean(-1, keepdim=True) + layernorm.variance_epsilon
        )
    ...
    if IS_GEMMA2:
        if HAS_FLASH_ATTENTION_SOFTCAPPING and attention_mask is None:
            ...
            if HAS_FLEX_ATTENTION:
                ...
                self.GA_mask = create_flex_attention_causal_mask(
                    self.max_seq_length
                )
            else:
...
def _LlamaModel_fast_forward_inference(
    ...
    def LlamaModel_fast_forward_inference_custom(
        ...
        residual = torch.empty(
            (bsz, q_len, hd), dtype=torch.float32, device="cuda:0"
        )
        _XX = torch.empty(
            (2, bsz, q_len, hd), dtype=torch.float32, device="cuda:0"
        )
        XX, XX2 = _XX[0], _XX[1]
        variance = torch.empty(
            (bsz, q_len, 1), dtype=torch.float32, device="cuda:0"
        )
        temp_mlp = torch.empty(
            (2, bsz, 1, mlp_size), dtype=X.dtype, device="cuda:0"
        )
...
class LlamaRotaryEmbedding(torch.nn.Module):
    ...
    def extend_rope_embedding(self, x, seq_len):
        ...
        self.current_rope_size = (
            (seq_len // 8192) + ((seq_len % 8192) != 0)
        ) * 8192
        self._set_cos_sin_cache(
            self.current_rope_size, device="cuda", dtype=x.dtype
        )
...
class LongRopeRotaryEmbedding(torch.nn.Module):
    ...
    def __init__(
        ...
        short_factor = torch.tensor(
            short_factor, device="cpu", dtype=torch.float32
        )
        long_factor = torch.tensor(
            long_factor, device="cpu", dtype=torch.float32
        )
        ...
        self.register_buffer(
            "short_inv_freq", short_inv_freq, persistent=False
        )
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        t = torch.arange(
            self.current_rope_size,
            device=self.long_inv_freq.device,
            dtype=torch.int64,
        ).float()
        # Long sequences
...
def unsloth_fast_generate(
    ...
    **kwargs,
    ...
    if model_eos_token_id is not None and hasattr(
        model_eos_token_id, "__iter__"
    ):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if not fast_inference:
            ...
            from atomgpt.inverse_models.vllm_utils import (
                load_vllm,
                get_vllm_state_dict,
            ...
            for allowed_arg in allowed_args:
                if (
                    allowed_arg not in load_vllm_kwargs
                    and allowed_arg in kwargs
                ):
        ...
        model.for_training = functools.partial(
            FastLlamaModel.for_training, model
        )
        model.for_inference = functools.partial(
            FastLlamaModel.for_inference, model
        )
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        if isinstance(model, PeftModelForCausalLM):
            ...
            if check_all:
                ...
                if "embed_tokens" in new_target_modules:
                    ...
                    model.get_input_embeddings().original_module.requires_grad_(
                        False
                    )
                pass
                ...
                if "lm_head" in new_target_modules:
                    ...
                    model.get_output_embeddings().original_module.requires_grad_(
                        False
                    )
                pass
        ...
        if vllm_engine is not None:
            ...
            from atomgpt.inverse_models.vllm_utils import save_lora, load_lora
        ...
        if lora_dropout == 0 and bias == "none":
            for idx, layer in enumerate(model.model.model.layers):
                ...
                if (
                    ...
                    and (
                        len(
                            getattr(gate_proj, "lora_magnitude_vector", [])
                            or []
                        )
                        == 0
                    )
                    and (
                        len(
                            getattr(up_proj, "lora_magnitude_vector", []) or []
                        )
                        == 0
                    )
                    and (
                        len(
                            getattr(down_proj, "lora_magnitude_vector", [])
                            or []
                        )
                        == 0
                    )
                ...
                if (
                    ...
                    and (
                        len(getattr(q_proj, "lora_magnitude_vector", []) or [])
                        == 0
                    )
                    and (
                        len(getattr(k_proj, "lora_magnitude_vector", []) or [])
                        == 0
                    )
                    and (
                        len(getattr(v_proj, "lora_magnitude_vector", []) or [])
                        == 0
                    )
                ):
                ...
                if (
                    ...
                    and (
                        len(getattr(o_proj, "lora_magnitude_vector", []) or [])
                        == 0
                    )
                ):
...
from atomgpt.inverse_models.rl import PatchFastRL

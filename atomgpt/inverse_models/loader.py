...
from atomgpt.inverse_models.qwen3 import FastQwen3Model
from atomgpt.inverse_models.qwen3_moe import FastQwen3MoeModel
from atomgpt.inverse_models.cohere import FastCohereModel
from transformers import AutoConfig
from transformers import __version__ as transformers_version
from peft import PeftConfig, PeftModel
from atomgpt.inverse_models.loader_utils import get_model_name
...
if SUPPORTS_GEMMA2:
    from atomgpt.inverse_models.gemma2 import FastGemma2Model
pass
import torch
from atomgpt.inverse_models._utils import (
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if token is None:
            token = get_token()
        assert (
            dtype is None or dtype == torch.float16 or dtype == torch.bfloat16
        )
        ...
        if model_type == "llama":
            ...
            if getattr(model_config, "rope_scaling", None) is not None:
                ...
                scaling_type2 = model_config.rope_scaling.get(
                    "rope_type", None
                )
                scaling_type = (
                    scaling_type1
                    if scaling_type1 is not None
                    else scaling_type2
                )
            pass
        ...
        if (
            ...
            and os.path.exists(
                os.path.join(old_model_name, "special_tokens_map.json")
            )
        ):
        ...
        if fast_inference:
            ...
            if model_name.endswith("unsloth-bnb-4bit"):
                if not vllm_dynamic_quant_supported(model_name, model_config):
                    ...
                    model_name = (
                        model_name[: -len("unsloth-bnb-4bit")] + "bnb-4bit"
                    )
                pass
            pass
        pass
...
class FastModel(FastBaseModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if "pixtral" in lowered_model_name and transformers_version < Version(
            "4.49.0"
        ):
            raise RuntimeError(
        ...
        # Check if VLM
        is_vlm = any(
            x.endswith("ForConditionalGeneration")
            for x in model_config.architectures
        )
        is_vlm = is_vlm or hasattr(model_config, "vision_config")
        if auto_model is None:
            auto_model = (
                AutoModelForVision2Seq if is_vlm else AutoModelForCausalLM
            )
